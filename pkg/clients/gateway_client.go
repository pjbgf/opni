...
func (gc *gatewayClient) Connect(ctx context.Context) (_ grpc.ClientConnInterface, errf future.Future[error]) {
	...
	for _, sc := range gc.spliced {
		...
		if err != nil {
			gc.logger.Warn("failed to connect to spliced stream, skipping", "name", name,
				logger.Err(err))
		...
		if err := ts.Splice(splicedStream,
			...
			gc.logger.Warn("failed to splice remote stream, skipping", "name", name,
				logger.Err(err))
		...
		defer func() {
			...
			if _, err := streamClient.Notify(ctx, &streamv1.StreamEvent{
				...
				gc.logger.Error("failed to notify remote stream", "name", name,
					logger.Err(err))
	...
	if f.IsSet() {
		gc.logger.Error("failed to connect to gateway", logger.Err(f.Get()))
