...
func (m *OpenidMiddleware) tryConfigureKeyRefresher(ctx context.Context) {
		...
		for backoff.Continue(b) {
			...
			if err != nil {
				if isDiscoveryErrFatal(err) {
					lg.Error("fatal error fetching openid configuration", logger.Err(err))
					panic(err)
	...
	if m.conf.Discovery != nil && m.conf.Discovery.CACert != nil {
		...
		if err != nil {
			lg.Error(
				...
				"openid discovery: failed to read CA cert", logger.Err(err),
	...
	if err != nil {
		lg.Error("failed to create user info cache", logger.Err(err))
		panic(err)
