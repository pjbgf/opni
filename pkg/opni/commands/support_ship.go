...
func shipRKERancher(ctx context.Context, cc grpc.ClientConnInterface, lg *slog.Logger) error {
	...
	for _, file := range files {
		...
		if err != nil {
			lg.Error(fmt.Sprintf("failed to publish rancher logs %s", file), logger.Err(err))
		}
		reader.Close()
	}
	return nil
...
func shipRKE2Etcd(ctx context.Context, cc grpc.ClientConnInterface, lg *slog.Logger) error {
	...
	for _, file := range files {
		...
		err = s.Publish(ctx, reader.Scan())
		if err != nil {
			lg.Error(fmt.Sprintf("failed to publish rke2 etcd logs %s", file), logger.Err(err))
		}
		reader.Close()
	}
...
func shipRKE2KubeAPI(ctx context.Context, cc grpc.ClientConnInterface, lg *slog.Logger) error {
	...
	for _, file := range files {
		...
		err = s.Publish(ctx, reader.Scan())
		if err != nil {
			lg.Error(fmt.Sprintf("failed to publish rke2 kube-api logs %s", file), logger.Err(err))
		}
		reader.Close()
	}
...
func shipRKE2KubeControllerManager(ctx context.Context, cc grpc.ClientConnInterface, lg *slog.Logger) error {
	...
	for _, file := range files {
		...
		err = s.Publish(ctx, reader.Scan())
		if err != nil {
			lg.Error(fmt.Sprintf("failed to publish rke2 kube-controller-manager logs %s", file), logger.Err(err))
		}
		reader.Close()
	}
...
func shipRKE2KubeScheduler(ctx context.Context, cc grpc.ClientConnInterface, lg *slog.Logger) error {
	...
	for _, file := range files {
		...
		err = s.Publish(ctx, reader.Scan())
		if err != nil {
			lg.Error(fmt.Sprintf("failed to publish rke2 kube-scheduler logs %s", file), logger.Err(err))
		}
		reader.Close()
	}
...
func shipRKE2KubeProxy(ctx context.Context, cc grpc.ClientConnInterface, lg *slog.Logger) error {
	...
	for _, file := range files {
		...
		err = s.Publish(ctx, reader.Scan())
		if err != nil {
			lg.Error(fmt.Sprintf("failed to publish rke2 kube-proxy logs %s", file), logger.Err(err))
		}
		reader.Close()
	}
...
func shipRKE2Rancher(ctx context.Context, cc grpc.ClientConnInterface, lg *slog.Logger) error {
	...
	for _, file := range files {
		...
		err = s.Publish(ctx, reader.Scan())
		if err != nil {
			lg.Error(fmt.Sprintf("failed to publish rke2 rancher logs %s", file), logger.Err(err))
		}
		reader.Close()
	}
