...
func (s *AlertingNode) updateConfig(ctx context.Context, config *node.AlertingCapabilityConfig) error {
	...
	if err != nil {
		s.lg.Error(fmt.Sprintf("failed to fetch %s node id %s", s.capability, err), logger.Err(err))
		return err
	}
	s.idMu.RUnlock()
	...
	if err := eg.Error(); err != nil {
		...
		s.lg.Error(fmt.Sprintf("%s node configuration error", s.capability), logger.Err(err))
		return err
	} else {
		s.config = config
	}
...
func (s *AlertingNode) sendHealthUpdate() {
	...
	if err != nil {
		s.lg.Warn(fmt.Sprintf("failed to send %s node health updates", s.capability), logger.Err(err))
		return
	}
